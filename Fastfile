...
platform :ios do
  ...
  scheme = "CCI_Fabric_Test"

  desc "Execute Unit test and call Crashlytics lane"
  lane :start_lane do |options|

    puts "*********************************************************"
    puts "Launching fastlane with parameters: #{options[:ipa_name]}"
    puts "*********************************************************"
  ...
  lane :CrashlyticsBeta do |options|
    ...
    puts "*********************************************************"
    puts "Starting building and distribuiting #{options[:ipa_name]}"
    puts "*********************************************************"
    ...
    crashlytics(
      ...
      ipa_path: "../CrashlyticsBetaBuilds/#{options[:ipa_name]}",
